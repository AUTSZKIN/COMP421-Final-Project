...
import matplotlib
import configuration.config_modified as cfg
...
def make_plots(context, predictions_timesteps, true_values, look_ahead, title, path, save_figure, Xserver):
    ...
    for idx, i in enumerate(np.arange(0, look_ahead, step)):
        ...
        plt.plot(true_values, label="True value", linewidth=1,
                 color=sns.xkcd_rgb["denim blue"])
        plt.plot(predictions_timesteps[:, i], label="Predicted value",
                 linewidth=1, linestyle="--", color=sns.xkcd_rgb["medium green"])
        error = abs(true_values - predictions_timesteps[:, i])
        plt.plot(error, label='Error',
                 color=sns.xkcd_rgb["pale red"], linewidth=0.5)
