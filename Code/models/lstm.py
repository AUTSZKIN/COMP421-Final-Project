...
class VanillaLSTM(object):
    ...
    def build_model(self):
        ...
        start = time.time()
        self.model.compile(
            loss=self.loss, optimizer=Adam(lr=self.learning_rate))
    ...
    def build_model(self):
        ...
        for i in range(2, self.n_hidden+1):
            ...
            if i == self.n_hidden:
                return_sequences = False
            self.model.add(
                LSTM(self.layers["hidden"+str(i)], return_sequences=return_sequences))
            self.model.add(Dropout(self.dropout))
        ...
        self.model.compile(loss=self.loss, optimizer=Adam(
            lr=self.learning_rate, decay=.99))
...
class StatefulMultiStepLSTM(object):
    ...
    def build_model(self):
        ...
        self.model.add(LSTM(
            ...
            batch_input_shape=(
                self.batch_size, self.look_back, self.layers['input']),
        ...
        for i in range(2, self.n_hidden + 1):
            ...
            if i == self.n_hidden:
                return_sequences = False
            self.model.add(LSTM(units=self.layers["hidden" + str(
                i)], stateful=True, return_sequences=return_sequences, unroll=True))
            self.model.add(Dropout(self.dropout))
...
def train_stateful_model(model, x_train, y_train, batch_size, epochs, shuffle, validation, validation_data, patience):
    ...
    logging.info('Training duration (s) : %s', str(
        time.time() - training_start_time))
    logging.info("Training Loss per epoch: %s" %
                 str(history_callback.history["loss"]))
    if validation:
        logging.info("Validation  Loss per epoch: %s" %
                     str(history_callback.history["val_loss"]))
...
def train_model(model, x_train, y_train, batch_size, epochs, shuffle, validation, validation_data, patience):
    ...
    if validation:
        ...
        history_callback = model.fit(
            x_train, y_train, batch_size=batch_size, epochs=epochs, shuffle=shuffle, verbose=2)
    logging.info('Training duration (s) : %s', str(
        time.time() - training_start_time))
    logging.info("Training Loss per epoch: %s" %
                 str(history_callback.history["loss"]))
